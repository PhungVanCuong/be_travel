<?php

namespace Database\Seeders;

use Illuminate\Database\Seeder;
use Illuminate\Support\Facades\DB;
use Carbon\Carbon;

class TourSeeder extends Seeder
{
    public function run(): void
    {
        $now = Carbon::now();

        $tours = [
            [
                'ten_tour' => 'Đà Nẵng: Khám phá Bà Nà Hills - Cầu Vàng',
                'mo_ta' => 'Trải nghiệm cáp treo đạt nhiều kỷ lục thế giới, check-in Cầu Vàng hùng vĩ và lạc lối trong không gian kiến trúc Làng Pháp cổ điển.',
                'gia' => 1250000.00,
                'ngay_bat_dau' => '2026-05-01',
                'ngay_ket_thuc' => '2026-05-01',
                'so_nguoi_toi_da' => 40,
                'diem_don' => 'Trung tâm TP. Đà Nẵng',
                'diem_tra' => 'Trung tâm TP. Đà Nẵng',
                'tinh_trang' => 1,
                'hinh_anh' => 'https://danang24.vn/Images/TV/18.jpg',
                'id_quoc_gia' => 1, // ID tương ứng với Việt Nam
                'created_at' => $now, 'updated_at' => $now,
            ],
            [
                'ten_tour' => 'Vịnh Hạ Long: Du thuyền & Chèo Kayak',
                'mo_ta' => 'Chiêm ngưỡng kỳ quan thiên nhiên thế giới từ du thuyền sang trọng, tham gia chèo thuyền Kayak khám phá các hang động kỳ ảo trên vịnh.',
                'gia' => 2500000.00,
                'ngay_bat_dau' => '2026-05-10',
                'ngay_ket_thuc' => '2026-05-11',
                'so_nguoi_toi_da' => 30,
                'diem_don' => 'Cảng tàu khách quốc tế Hạ Long',
                'diem_tra' => 'Cảng tàu khách quốc tế Hạ Long',
                'tinh_trang' => 1,
                'hinh_anh' => 'https://ik.imagekit.io/tvlk/xpe-asset/AyJ40ZAo1DOyPyKLZ9c3RGQHTP2oT4ZXW+QmPVVkFQiXFSv42UaHGzSmaSzQ8DO5QIbWPZuF+VkYVRk6gh-Vg4ECbfuQRQ4pHjWJ5Rmbtkk=/5596295958911/Ha-Long-Bay-Day-Cruise-6-Star-All-In-with-Buffet-Kayak-Sunset-Party-Titop-2676835a-ad56-43b3-8f67-1f6898f5010b.png?tr=q-60,c-at_max,w-1280,h-720&_src=imagekit',
                'id_quoc_gia' => 1,
                'created_at' => $now, 'updated_at' => $now,
            ],
            [
                'ten_tour' => 'Phú Quốc: Bãi Sao & Cáp treo Hòn Thơm',
                'mo_ta' => 'Tận hưởng làn nước xanh ngắt tại Bãi Sao và trải nghiệm tuyến cáp treo vượt biển dài nhất thế giới sang đảo Hòn Thơm.',
                'gia' => 3800000.00,
                'ngay_bat_dau' => '2026-06-15',
                'ngay_ket_thuc' => '2026-06-17',
                'so_nguoi_toi_da' => 25,
                'diem_don' => 'Sân bay Phú Quốc',
                'diem_tra' => 'Sân bay Phú Quốc',
                'tinh_trang' => 1,
                'hinh_anh' => 'https://s3-cmc.travel.com.vn/vtv-image/Images/Tour/tfd_240131095355_074901_SUN%20WORLD%20HON%20THOM%20(4).jpg',
                'id_quoc_gia' => 1,
                'created_at' => $now, 'updated_at' => $now,
            ],
            [
                'ten_tour' => 'Tà Xùa: Săn mây & Sống lưng khủng long',
                'mo_ta' => 'Hành trình chinh phục thiên đường mây Tà Xùa, check-in "Sống lưng khủng long" và tận hưởng không khí hùng vĩ của núi rừng Tây Bắc.',
                'gia' => 1850000.00,
                'ngay_bat_dau' => '2026-04-20',
                'ngay_ket_thuc' => '2026-04-22',
                'so_nguoi_toi_da' => 15,
                'diem_don' => 'Bến xe Mỹ Đình, Hà Nội',
                'diem_tra' => 'Bến xe Mỹ Đình, Hà Nội',
                'tinh_trang' => 1,
                'hinh_anh' => 'https://thdtravel.com.vn/wp-content/uploads/2025/11/song-lung-khung-long-ta-xua-hang-dong-son-la-du-lich-de-men-vn.webp',
                'id_quoc_gia' => 1,
                'created_at' => $now, 'updated_at' => $now,
            ],
            [
                'ten_tour' => 'Di sản miền Trung: Đại Nội Huế - Hội An',
                'mo_ta' => 'Tìm về cội nguồn văn hóa tại Cố đô Huế và thả mình trong không gian lung linh của Phố cổ Hội An về đêm.',
                'gia' => 2900000.00,
                'ngay_bat_dau' => '2026-05-05',
                'ngay_ket_thuc' => '2026-05-07',
                'so_nguoi_toi_da' => 20,
                'diem_don' => 'Ga Huế',
                'diem_tra' => 'Phố cổ Hội An',
                'tinh_trang' => 1,
                'hinh_anh' => 'https://www.arttravel.com.vn/upload/sanpham/hue-4-9377-6772.jpg',
                'id_quoc_gia' => 1,
                'created_at' => $now, 'updated_at' => $now,
            ],
            [
                'ten_tour' => 'Đà Lạt: Thung lũng Tình Yêu & Quảng trường Lâm Viên',
                'mo_ta' => 'Thăm thành phố ngàn hoa, check-in nụ hoa Atiso khổng lồ tại Quảng trường và dạo bước trong Thung lũng Tình Yêu thơ mộng.',
                'gia' => 3200000.00,
                'ngay_bat_dau' => '2026-05-12',
                'ngay_ket_thuc' => '2026-05-15',
                'so_nguoi_toi_da' => 30,
                'diem_don' => 'Sân bay Liên Khương',
                'diem_tra' => 'Trung tâm Đà Lạt',
                'tinh_trang' => 1,
                'hinh_anh' => 'https://res.klook.com/image/upload/w_750,h_469,c_fill,q_85/w_80,x_15,y_15,g_south_west,l_Klook_water_br_trans_yhcmh3/activities/ukykclh6jireescsohoj.jpg',
                'id_quoc_gia' => 1,
                'created_at' => $now, 'updated_at' => $now,
            ],
            [
                'ten_tour' => 'Miền Tây: Chợ nổi Cái Răng & Miệt vườn trái cây',
                'mo_ta' => 'Trải nghiệm nét văn hóa sông nước độc đáo tại chợ nổi lớn nhất miền Tây và thưởng thức trái cây tươi ngon ngay tại vườn.',
                'gia' => 1500000.00,
                'ngay_bat_dau' => '2026-06-20',
                'ngay_ket_thuc' => '2026-06-21',
                'so_nguoi_toi_da' => 35,
                'diem_don' => 'Bến Ninh Kiều, Cần Thơ',
                'diem_tra' => 'Bến Ninh Kiều, Cần Thơ',
                'tinh_trang' => 1,
                'hinh_anh' => 'https://sundaygo.vn/wp-content/uploads/2025/02/tour-cho-noi-cai-rang-can-tho.jpg',
                'id_quoc_gia' => 1,
                'created_at' => $now, 'updated_at' => $now,
            ],
            [
                'ten_tour' => 'Sapa: Chinh phục Fansipan & Bản Cát Cát',
                'mo_ta' => 'Chạm tay vào cột mốc "Nóc nhà Đông Dương" và tìm hiểu đời sống văn hóa đặc sắc của người dân tộc Mông tại bản Cát Cát.',
                'gia' => 4500000.00,
                'ngay_bat_dau' => '2026-05-18',
                'ngay_ket_thuc' => '2026-05-20',
                'so_nguoi_toi_da' => 20,
                'diem_don' => 'Ga Lào Cai',
                'diem_tra' => 'Thị xã Sapa',
                'tinh_trang' => 1,
                'hinh_anh' => 'https://www.hoangviettravel.com.vn/sites/default/files/styles/origin/public/hinh-anh-sapa-moana-fansipan-ban-cat-cat-3n2d.jpg?itok=HHc8rHrU',
                'id_quoc_gia' => 1,
                'created_at' => $now, 'updated_at' => $now,
            ],
            [
                'ten_tour' => 'Nha Trang: Lặn ngắm san hô & Tắm bùn Hòn Tằm',
                'mo_ta' => 'Khám phá thế giới đại dương rực rỡ và thư giãn tuyệt đối với dịch vụ tắm bùn khoáng nóng trên đảo Hòn Tằm.',
                'gia' => 2200000.00,
                'ngay_bat_dau' => '2026-07-01',
                'ngay_ket_thuc' => '2026-07-02',
                'so_nguoi_toi_da' => 25,
                'diem_don' => 'Cảng Cầu Đá, Nha Trang',
                'diem_tra' => 'Cảng Cầu Đá, Nha Trang',
                'tinh_trang' => 1,
                'hinh_anh' => 'https://statics.vinpearl.com/lan-bien-nha-trang-03_1635674598.jpg',
                'id_quoc_gia' => 1,
                'created_at' => $now, 'updated_at' => $now,
            ],
            [
                'ten_tour' => 'Ninh Bình: Danh thắng Tràng An & Chùa Bái Đính',
                'mo_ta' => 'Ngồi thuyền xuôi dòng Ngô Đồng khám phá di sản thế giới Tràng An và chiêm bái ngôi chùa có nhiều kỷ lục nhất Việt Nam.',
                'gia' => 1350000.00,
                'ngay_bat_dau' => '2026-05-22',
                'ngay_ket_thuc' => '2026-05-22',
                'so_nguoi_toi_da' => 45,
                'diem_don' => 'Thành phố Ninh Bình',
                'diem_tra' => 'Thành phố Ninh Bình',
                'tinh_trang' => 1,
                'hinh_anh' => 'https://vivuhalong.com/wp-content/uploads/2024/08/Chua-Bai-Dinh-Ninh-Binh-3.jpg',
                'id_quoc_gia' => 1,
                'created_at' => $now, 'updated_at' => $now,
            ],
            [
                'ten_tour' => 'Chiang Mai - Chiang Rai: Đóa hồng phương Bắc',
                'mo_ta' => 'Viếng thăm Chùa Trắng Wat Rong Khun tuyệt đẹp, khám phá Làng cổ dài và tận hưởng không khí yên bình của vùng núi phía Bắc Thái Lan.',
                'gia' => 7500000.00,
                'ngay_bat_dau' => '2026-08-12',
                'ngay_ket_thuc' => '2026-08-16',
                'so_nguoi_toi_da' => 25,
                'diem_don' => 'Sân bay quốc tế Chiang Mai',
                'diem_tra' => 'Sân bay quốc tế Chiang Mai',
                'tinh_trang' => 1,
                'hinh_anh' => 'https://www.hoangviettravel.com.vn/sites/default/files/hinh-anh-kham-pha-chiangmai-chiangrai-doa-hong-phuong-bac-thai-lan.jpg',
                'id_quoc_gia' => 2,
                'created_at' => $now, 'updated_at' => $now,
            ],
            [
                'ten_tour' => 'Phuket - Đảo Phi Phi: Thiên đường biển đảo',
                'mo_ta' => 'Nghỉ dưỡng tại hòn đảo ngọc Phuket, lặn ngắm san hô tại vịnh Maya và thưởng thức hải sản tươi ngon trên bãi biển cát trắng.',
                'gia' => 8200000.00,
                'ngay_bat_dau' => '2026-05-20',
                'ngay_ket_thuc' => '2026-05-23',
                'so_nguoi_toi_da' => 20,
                'diem_don' => 'Sân bay quốc tế Phuket',
                'diem_tra' => 'Sân bay quốc tế Phuket',
                'tinh_trang' => 1,
                'hinh_anh' => 'https://haidangtravel.com/image/tour-du-lich-phuket-thien-duong-bien-dao-dao-phi-phi-hinh-2.jpg',
                'id_quoc_gia' => 2,
                'created_at' => $now, 'updated_at' => $now,
            ],
            [
                'ten_tour' => 'Busan - Làng văn hóa Gamcheon',
                'mo_ta' => 'Khám phá thành phố cảng lớn nhất Hàn Quốc, dạo bước qua những ngôi nhà rực rỡ sắc màu tại Gamcheon và ngắm cảnh biển Haeundae.',
                'gia' => 11000000.00,
                'ngay_bat_dau' => '2026-09-05',
                'ngay_ket_thuc' => '2026-09-09',
                'so_nguoi_toi_da' => 30,
                'diem_don' => 'Sân bay quốc tế Gimhae, Busan',
                'diem_tra' => 'Sân bay quốc tế Gimhae, Busan',
                'tinh_trang' => 1,
                'hinh_anh' => 'https://thaiantravel.com/wp-content/uploads/2025/01/avt-lang-van-hoa-Gamcheon.webp',
                'id_quoc_gia' => 3,
                'created_at' => $now, 'updated_at' => $now,
            ],
            [
                'ten_tour' => 'Đảo Jeju: Hòn đảo hòa bình',
                'mo_ta' => 'Chinh phục đỉnh núi Seongsan Ilchulbong, thăm quan Thác nước Cheonjiyeon và đắm mình trong cánh đồng hoa cải vàng rực rỡ.',
                'gia' => 13500000.00,
                'ngay_bat_dau' => '2026-04-10',
                'ngay_ket_thuc' => '2026-04-14',
                'so_nguoi_toi_da' => 20,
                'diem_don' => 'Sân bay quốc tế Jeju',
                'diem_tra' => 'Sân bay quốc tế Jeju',
                'tinh_trang' => 1,
                'hinh_anh' => 'https://thaiantravel.com/wp-content/uploads/2025/01/dao-jeju-1.webp',
                'id_quoc_gia' => 3,
                'created_at' => $now, 'updated_at' => $now,
            ],
            [
                'ten_tour' => 'Hokkaido: Mùa đông lãng mạn & Trượt tuyết Niseko',
                'mo_ta' => 'Trải nghiệm mùa đông tuyết trắng tại Hokkaido, tham gia trượt tuyết đỉnh cao tại Niseko và thưởng thức cua tuyết hoàng đế trứ danh.',
                'gia' => 28000000.00,
                'ngay_bat_dau' => '2026-12-15',
                'ngay_ket_thuc' => '2026-12-20',
                'so_nguoi_toi_da' => 15,
                'diem_don' => 'Sân bay Shin-Chitose, Sapporo',
                'diem_tra' => 'Sân bay Shin-Chitose, Sapporo',
                'tinh_trang' => 1,
                'hinh_anh' => 'https://trieuhaotravel.vn/Uploads/images/leech/gdocs_1_20251003_092325_0d88c791.png',
                'id_quoc_gia' => 4,
                'created_at' => $now, 'updated_at' => $now,
            ],
            [
                'ten_tour' => 'Okinawa: Hawaii của Châu Á',
                'mo_ta' => 'Nghỉ dưỡng tại vùng biển xanh trong vắt, lặn ngắm san hô, thăm Thủy cung Churaumi lớn nhất thế giới và tìm hiểu văn hóa Ryukyu.',
                'gia' => 21000000.00,
                'ngay_bat_dau' => '2026-06-05',
                'ngay_ket_thuc' => '2026-06-09',
                'so_nguoi_toi_da' => 20,
                'diem_don' => 'Sân bay Naha, Okinawa',
                'diem_tra' => 'Sân bay Naha, Okinawa',
                'tinh_trang' => 1,
                'hinh_anh' => 'https://nippontravel.vn/wp-content/uploads/2023/07/okinawa-mua-he.jpg',
                'id_quoc_gia' => 4,
                'created_at' => $now, 'updated_at' => $now,
            ],
            [
                'ten_tour' => 'Singapore: Khám phá hoang dã tại Night Safari',
                'mo_ta' => 'Khám phá sở thú đêm đầu tiên trên thế giới, ngồi xe điện ngắm nhìn các loài động vật hoang dã và chiêm ngưỡng thác nước trong nhà tại Jewel Changi.',
                'gia' => 7500000.00,
                'ngay_bat_dau' => '2026-08-01',
                'ngay_ket_thuc' => '2026-08-03',
                'so_nguoi_toi_da' => 40,
                'diem_don' => 'Sân bay Quốc tế Changi, Singapore',
                'diem_tra' => 'Sân bay Quốc tế Changi, Singapore',
                'tinh_trang' => 1,
                'hinh_anh' => 'https://mia.vn/media/uploads/blog-du-lich/tham-quan-night-safari-singapore-01-1710111449.jpeg',
                'id_quoc_gia' => 5,
                'created_at' => $now, 'updated_at' => $now,
            ],
            [
                'ten_tour' => 'Singapore: Trải nghiệm Du thuyền xa hoa',
                'mo_ta' => 'Nghỉ dưỡng 3 ngày 2 đêm trên siêu du thuyền Royal Caribbean, thưởng thức show diễn đẳng cấp và tận hưởng các tiện ích 5 sao giữa đại dương.',
                'gia' => 14500000.00,
                'ngay_bat_dau' => '2026-11-10',
                'ngay_ket_thuc' => '2026-11-12',
                'so_nguoi_toi_da' => 50,
                'diem_don' => 'Cảng Marina Bay Cruise Centre',
                'diem_tra' => 'Cảng Marina Bay Cruise Centre',
                'tinh_trang' => 1,
                'hinh_anh' => 'https://media.vietravel.com/images/Content/trai-nghiem-du-thuyen-o-singapore-4.png',
                'id_quoc_gia' => 5,
                'created_at' => $now, 'updated_at' => $now,
            ]
        ];

        DB::table('tours')->truncate();
        DB::table('tours')->delete();
        DB::table('tours')->insert($tours);
    }
}
